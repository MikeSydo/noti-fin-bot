...
async def discover_database_ids(access_token: str, duplicated_template_id: str) -> Optional[dict]:
    ...
    # Database title keywords to look for (supports English and Ukrainian template names)
    db_mapping = {
        "Accounts": "accounts_db_id",
        "Акаунти": "accounts_db_id",
        
        "Expenses": "expenses_db_id",
        "Витрати": "expenses_db_id",
        
        "Group Expenses": "group_expenses_db_id",
        "Спільні витрати": "group_expenses_db_id",
        
        "Categories": "categories_db_id",
        "Категорії": "categories_db_id",
    }
    result = {}

    async with aiohttp.ClientSession() as session:
        # Get children of the duplicated template page
        url = f"https://api.notion.com/v1/blocks/{duplicated_template_id}/children?page_size=100"
        async with session.get(url, headers=headers) as resp:
            if resp.status != 200:
                logger.error(f"Failed to get template children: {await resp.text()}")
                return None

            data = await resp.json()

        # Search for child_database blocks
    ...
    found = len(result)
    expected = len(db_mapping)
    if found < expected:
        logger.warning(f"Only found {found}/{expected} databases in template. Found: {list(result.keys())}")
...
async def process_oauth_callback(code: str, telegram_id: int) -> bool:
    ...
    # Discover database IDs from duplicated template
    if duplicated_template_id:
        db_ids = await discover_database_ids(access_token, duplicated_template_id)
        if db_ids:
            update_data.update(db_ids)
            logger.info(f"Discovered {len(db_ids)} databases for user {telegram_id}")
        else:
            logger.warning(f"Could not discover databases from template for user {telegram_id}")
    ...
    logger.info(f"OAuth flow completed for user {telegram_id} (workspace: {workspace_name})")
